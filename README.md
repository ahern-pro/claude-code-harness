...
- Master-Worker模式：master agent是主要执行者，将阻塞 / 只读任务（如：只读搜索代码、review代码）可选委派给worker；适合小型任务
- Coordinator（协调者）模式：主agent担任编排者，将任务拆分成工作流，并把子任务委派给子agent；适合大型任务
